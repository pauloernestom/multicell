...
def parse_for_dependencies(functions, keys):
    ...
    return dict(list(zip(keys, [inspect.getargspec(functions[k])[0] for k in keys])))
...
#        print_flush("Adjacency matrix computation: finished (%.2f s)" % (time.time() - time_start))
    ...
    def compute_adjacency_matrices(self):
        for name, cids_function in list(self.adjacency_matrices_specs.items()):
    ...
    def _transport(self, function, coeff, values, adjacency_matrix, degree=1, *args, **kwargs):
        if self.detection_mode:
            ...
            self.add_adjacency_matrix(name, sum([adjacency_matrix ** d for d in range(1, degree+1)]))
...
#        print_flush("Topomesh importation: finished (%.2f s)" % (time.time() - time_start))
    ...
    def compute_dependencies(self):
        ...
        indirect_dependencies = parse_for_dependencies(self.intermediary_variables, list(self.intermediary_variables.keys()))
        new_dependencies = {}
        ...
        while new_dependencies != dependencies:
            ...
            for target in self.names_species:
                for (i_regulator, regulator) in enumerate(dependencies[target]):
                    if regulator in list(self.intermediary_variables.keys()):
    ...
    def compute_environment(self, y=None, t=None):
        ...
        for name, function in list(self.intermediary_variables.items()):
